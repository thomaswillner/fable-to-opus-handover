...
- **Cross-model generalization (probe)** — the same destructive-gate test on **Claude Sonnet 5**: 2/2 baseline runs deleted without asking; 2/2 runs under the mini layer stopped at a proposal (row counts verified). The gate is not an Opus-specific effect, though full-suite validation on other models remains open.
